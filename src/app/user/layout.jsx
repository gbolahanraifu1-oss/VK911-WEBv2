    ...
    <div style={{ minHeight: "100vh", background: "#080810", fontFamily: "'Inter', sans-serif" }}>
      ...
      <header style={{ background: "#0a0a16", borderBottom: "1px solid rgba(255,255,255,0.05)", padding: "0 24px", display: "flex", alignItems: "center", height: "56px", gap: "16px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginRight: "auto" }}>
          ...
          <span style={{ fontSize: "14px", fontWeight: "800", color: "#f1f5f9" }}>VK911 XMD</span>
        ...
        {navItems.map((item) => (
          <a key={item.id} href={item.href} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "6px 14px", borderRadius: "8px", background: active === item.id ? "rgba(0,255,136,0.1)" : "transparent", border: active === item.id ? "1px solid rgba(0,255,136,0.2)" : "1px solid transparent", color: active === item.id ? "#00ff88" : "#64748b", textDecoration: "none", fontSize: "13px", fontWeight: active === item.id ? "600" : "400" }}>
            <span>{item.icon}</span>
            <span style={{ display: "none" }} className="nav-label">{item.label}</span>
          </a>
        ))}

        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginLeft: "8px", paddingLeft: "16px", borderLeft: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <span style={{ fontSize: "12px", color: "#94a3b8" }}>{user?.username}</span>
          <button onClick={logout} style={{ padding: "5px 10px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: "6px", color: "#ef4444", fontSize: "11px", cursor: "pointer" }}>
            Sign Out
          </button>
        </div>
      </header>
